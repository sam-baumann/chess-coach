...
/* "13.Bxc6+", "13... Qc1+", "13. O-O". The dots carry the side to move, which
   ...
   The SAN is captured, not just matched: the coach writes the move it wishes had
   been played as often as the one that was, and those two need opposite
   treatment on the board. Without the SAN in hand, "10.a3" and "10.Qb3" are the
   same reference. */
...
export type Ref =
  ...
  | {
      ...
      moveNumber: number;
      black: boolean | null;
      /** The move itself ("Bg5"), or null for a bare "move 13" that names none. */
      san: string | null;
    };
...
export function findRefs(text: string): Ref[] {
  ...
  for (let m = REF.exec(text); m; m = REF.exec(text)) {
    const [raw, fen, numbered, dots, san, bare] = m;
    const span = { start: m.index, end: m.index + raw.length, raw };

    if (fen !== undefined) {
      out.push({ kind: "fen", ...span });
    } else if (numbered !== undefined) {
      out.push({ kind: "move", ...span, moveNumber: Number(numbered), black: dots === "...", san });
    } else if (bare !== undefined) {
      out.push({ kind: "move", ...span, moveNumber: Number(bare), black: null, san: null });
    }
  }
  return out;
}
