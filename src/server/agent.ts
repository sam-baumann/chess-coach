...
function hubContext(game: Game, scanRelPath: string | null): string {
  ...
  lines.push(
    ...
    "",
    "Two things you write can move the board beside the chat, and both are deliberate:",
    "nothing is guessed from your prose. Notation you do not tag is simply text — which",
    "is what lets you write `dxc4` and \"the d5 pawn\" in the same sentence.",
    "",
    "**A tagged move.** Write it as a markdown link whose target is",
    "`<line>:<move number>:<side>`, with `.` for White and `..` for Black:",
    "",
    "- `[14.e5](game:14:.)` — the move actually played. Clicking moves the scrubber.",
    "- `[15.Nxd4](A:15:.)` — White's 15th in the line you declared as `A`. Clicking puts",
    "  that position on the board, framed, since the game never reached it.",
    "",
    "The link text is yours — write the move the way the sentence wants it (`14... d5`,",
    "`dxc4`, `15.Nxd4!?`). It is checked against the move the tag names: if the two",
    "disagree the link is dropped and your text is left as written, so a mistagged move",
    "costs a link rather than showing a position the sentence is not about.",
    "",
    "**A FEN in backticks** is a link to that position too. Rarely needed now that every",
    "move of a line is reachable on its own.",
    "",
    "### Declaring a line",
    "",
    "Before tagging moves of a line the game did not play, declare the line, with an id:",
    "",
    "```line id=A",
    "14... d5 15.Nxd4 dxc4 16.bxc4",
    "```",
    "",
    "Then write the prose as you would anyway, tagging each move as it comes up:",
    '"the refutation is [14... d5](A:14:..), and after [15.Nxd4](A:15:.) [dxc4](A:15:..)',
    '[16.bxc4](A:16:.) the centre has gone."',
    "",
    "- **Paste the moves from the engine.** `probe_moments.py` prints its lines in exactly",
    "  this format — `14...d5 15.Nxd4 dxc4 16.bxc4 Bd7 17.Re3` — so copy the PV rather",
    "  than retyping it. A declared line should be one the engine gave you.",
    "- The reader never sees the block. It exists so the board can replay the line.",
    "- The first move needs its number and dots; the moves after it may be bare.",
    "- Moves only — no evaluations, no words. One bad token drops the whole block, and",
    "  then every tag into it stops resolving.",
    "- A line continuing another: declare the first with the info string `line id=A`, the",
    "  continuation with `line id=B from=A@Bxd4`, and only the moves after that branch.",
    "- Up to 24 half-moves per line.",
    "",
    "A move the game did play needs no block, only the `game` tag. Untagged notation —",
    "and phrases like \"around move 13\" — put nothing on the board.",
    "",
    "### Formatting",
    "",
    "The chat renders a deliberately small subset of markdown: headings, bullet and",
    "numbered lists, blockquotes, rules, bold, italic, links, and code. **Do not write",
    "markdown tables** — there is no table renderer, so a table arrives as a wall of",
    "raw pipes. When you would reach for one, write a bulleted list instead, one bullet",
    "per row, with the columns as a bolded lead-in and prose after it.",
    "",
    "### Finishing",
    "",
